"""
SwinUMambaD network architecture — redistributed from Swin-UMamba.

Original work: Liu et al., "Swin-UMamba: Mamba-based UNet with ImageNet-based
pretraining" (2024).
Original source: https://github.com/JiarunLiu/Swin-UMamba (Apache 2.0).
The upstream LICENSE is preserved at THIRD_PARTY_LICENSES/Swin-UMamba-LICENSE.

The encoder, decoder, VSS, and SS2D classes in this file are byte-identical to
their definitions in `swin_umamba/nnunetv2/nets/SwinUMambaD.py` of the upstream
repo. Please cite the original paper when using this code.

Modifications relative to the upstream file:
  - Removed nnUNet imports (`nnunetv2.*`, `dynamic_network_architectures.*`)
    so this module runs standalone without nnUNet installed.
  - Inlined `InitWeights_He` and `init_last_bn_before_add_to_0` as minimal
    standalone implementations.
  - Removed `get_swin_umamba_d_from_plans()` (which depended on nnUNet's
    PlansManager) and added a plain `build_swin_umamba_d()` constructor that
    takes ordinary Python arguments.
"""

import torch.nn as nn

class InitWeights_He:
    """Minimal stand-in for nnUNet's InitWeights_He."""
    ...
    def __call__(self, module: nn.Module) -> None:
        if isinstance(
            ...
            nn.init.kaiming_normal_(
                module.weight, a=self.neg_slope, nonlinearity="leaky_relu"
            )
            if module.bias is not None:
                module.bias.data.zero_()


def init_last_bn_before_add_to_0(module: nn.Module) -> None:
    """No-op stand-in; only meaningful inside nnUNet's specific init flow."""
    return None
