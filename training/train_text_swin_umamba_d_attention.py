...
Features:
  - YAML config (configs/isic2017_text_swin_umamba_d.yaml).
...
from src.models.text_swin_umamba_d_attention import build_text_swin_umamba_d_attention
...
def main():
    ...
    model_builder = build_text_swin_umamba_d_attention
    model_kwargs = dict(
        ...
        text_dim=text_dim,
        attention_heads=cfg["model"]["attention"].get("attention_heads", 4),
        attention_beta_init=cfg["model"]["attention"].get("beta_init", 0.1),
        attention_enabled=cfg["model"]["attention"].get("enabled", True),
    ...
    for epoch in range(start_epoch, cfg["train"]["epochs"]):
        ...
        for micro_step, batch in enumerate(train_loader):
            ...
            if text_mode == "tokens":
                ...
                with torch.no_grad():
                    text_pooled, text_tokens = text_encoder(input_ids, attn_mask)
            else:
                text_pooled = batch["text_pooled"].to(device, non_blocking=True)
                text_tokens = batch["text_tokens"].to(device, non_blocking=True)
                attn_mask = batch["text_attention_mask"].to(device, non_blocking=True)

            if micro_step % accum_steps == 0:
                optimizer.zero_grad(set_to_none=True)

            with torch.cuda.amp.autocast(enabled=use_amp):
                output = model(image, text_pooled, text_tokens, attn_mask)
        ...
        save_checkpoint(
            ...
            monitor_metric="ema_dice", monitor_mode="max",
        ...
        if improved and cfg["output"]["keep_best"]:
            ...
            print(
                ...
                f"(epoch {best_epoch}, ema_dice {best_ema_dice:.4f}, raw_f1_or_dsc {best_val_dice:.4f})"
            )
