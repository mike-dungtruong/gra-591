"""Training entry point for boundary-aware SwinUMamba on ISIC.

Usage:
    python training/train_boundary_swin_umamba.py --config configs/isic2017_boundary_swin_umamba.yaml
    python training/train_boundary_swin_umamba.py --config configs/isic2017_boundary_swin_umamba.yaml --resume auto
"""
...
from src.models.boundary_swin_umamba import build_boundary_swin_umamba
...
from src.utils.losses import BoundaryAwareSegLossWithDeepSupervision
...
def main():
    ...
    attach_text_log(run_dir / "training_log.txt")
    ...
    model = build_boundary_swin_umamba(
    ...
    print(
        f"[info] model params: {count_parameters(model)/1e6:.2f}M "
    ...
    criterion = BoundaryAwareSegLossWithDeepSupervision(
        ...
        boundary_kernel_size=cfg["loss"]["boundary_kernel_size"],
    ...
    for epoch in range(start_epoch, cfg["train"]["epochs"]):
        ...
        for micro_step, batch in enumerate(train_loader):
            ...
            is_update_step = (micro_step + 1) % accum_steps == 0 or (
                micro_step + 1 == len(train_loader)
            )
            if is_update_step:
        ...
        if budget.exhausted():
            print(
                f"[info] wall-clock budget exhausted ({budget.elapsed_hours():.2f}h). "
